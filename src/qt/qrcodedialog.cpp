...
{
    QString ret = QString("bitpagar:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non BPC unit input in lnReqAmount, we generate the URI with BPC as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::BPC, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
